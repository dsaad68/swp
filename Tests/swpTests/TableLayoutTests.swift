...
final class TableLayoutTests: XCTestCase {
    ...
    func testColumnsAreCappedAgainstOutliers() {
        ...
        let widths = TableLayout.widths(for: [outlier], totalWidth: 200)
    ...
    func testRowsFitTheWidthTheyWereSizedFor() {
        ...
        for width in [44, 60, 80, 120, 200] {
            let widths = TableLayout.widths(for: records, totalWidth: width)
            for record in records {
                let row = TableLayout.plainRow(cells: TableLayout.cells(for: record), widths: widths)
                XCTAssertEqual(Ansi.displayWidth(row), width,
                               "row should fill exactly \(width) columns")
            }
            XCTAssertEqual(Ansi.displayWidth(TableLayout.plainHeader(widths: widths)), width)
        }
    }
    ...
    func testBelowTheFloorTheLayoutStopsShrinking() {
        ...
        let narrow = TableLayout.widths(for: [record], totalWidth: 20)
        let row = TableLayout.plainRow(cells: TableLayout.cells(for: record), widths: narrow)
    ...
    func testNumericColumnsAreRightAligned() {
        let widths = TableLayout.widths(for: [makeProcess(pid: 7)], totalWidth: 100)
    ...
    func testStylingIsAbsentWhenColourIsOff() {
        Ansi.colorEnabled = false
        let record = makeProcess(ports: [3000])
        let widths = TableLayout.widths(for: [record], totalWidth: 80)
        let plain = TableLayout.plainRow(cells: TableLayout.cells(for: record), widths: widths)
        XCTAssertEqual(TableLayout.styledRow(plain, widths: widths, record: record, theme: .dark),
                       plain)
    }
    ...
    func testStylingPreservesTheVisibleText() {
        ...
        let widths = TableLayout.widths(for: [record], totalWidth: 80)
        let plain = TableLayout.plainRow(cells: TableLayout.cells(for: record), widths: widths)
        let styled = TableLayout.styledRow(plain, widths: widths, record: record, theme: .dark,
                                           highlight: [0, 1, 20])
    ...
    func testStylingEmitsRunsRatherThanPerCharacterEscapes() {
        ...
        defer { Ansi.colorEnabled = false }
        let record = makeProcess(ports: [3000])
        let widths = TableLayout.widths(for: [record], totalWidth: 120)
        let plain = TableLayout.plainRow(cells: TableLayout.cells(for: record), widths: widths)
        let styled = TableLayout.styledRow(plain, widths: widths, record: record, theme: .dark)
        let escapes = styled.components(separatedBy: "\u{1B}[").count - 1
        XCTAssertLessThanOrEqual(escapes, TableLayout.headers.count + 1)
    }
}
